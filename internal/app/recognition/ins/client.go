...
// 从当前包 internal/app/recognition/ins 往上四级就是项目根目录。
//
...
//	defer client.Close()
//
//	result, err := client.AnalyzeFrame(ctx, frameBytes)
//
...
// AnalyzeFrame 分析视频流中的单帧。
//
// imgBytes 是完整图片文件内容，不是裸 RGB/BGR 像素。
// 返回的 Box 是扁平数组，每张脸 4 个 float64：[x, y, width, height]。
func (a *Inspire) AnalyzeFrame(ctx context.Context, imgBytes []byte) (*recognition.FaceResult, error) {
...
// AnalyzeFramePose 分析视频流中的单帧，并返回 yaw/pitch/roll。
func (a *Inspire) AnalyzeFramePose(ctx context.Context, imgBytes []byte) (*recognition.FaceResult, error) {
...
func defaultConfig() Config {
	return Config{
		...
		MinFacePixels:    32,
		SessionCount:     1,
		Features:         FeatureFlags{Recognition: true, Quality: true, Pose: true},
	}
}
